...
class ArticleTests(TestCase):
    ...
    def test_meta_order(self):
        ...
        article1 = Article.objects.create(title='title1', body='article', status='p',
                                          topped=True, created_time=time1, last_modified_time=time1 + delta_time_30s)
        article2 = Article.objects.create(title='title2', body='article', status='p', topped=True,
                                          created_time=time1 + delta_time_20s, last_modified_time=time1 + delta_time_20s)
        article3 = Article.objects.create(title='title3', body='article', status='p',
                                          topped=False, created_time=time1, last_modified_time=time1 + delta_time_30s)
        article4 = Article.objects.create(title='title4', body='article', status='p',
                                          topped=False, created_time=time1, last_modified_time=time1 + delta_time_20s)
        article5 = Article.objects.create(title='title5', body='article', status='p', topped=False,
                                          created_time=time1 + delta_time_20s, last_modified_time=time1 + delta_time_20s)
        self.assertQuerysetEqual(Article.objects.all(), [
                                 '<Article: title2>', '<Article: title1>', '<Article: title5>', '<Article: title3>', '<Article: title4>'])
    ...
    def test_save(self):
        ...
        article2 = Article.objects.create(
            title='title2', body='article', abstract='', status='p')
        article3 = Article.objects.create(
            title='title3', body=body_with_53str, status='p')
        article4 = Article.objects.create(
            title='title4', body=body_with_55str, status='p')
    ...
    def test_manage_archive(self):
        ...
        try:
            time_2011_8 = datetime.datetime(
                2011, 8, 22, 11, 23, 45, 564, tzinfo=pytz.utc)
            time_2015_6 = datetime.datetime(
                2015, 6, 22, 11, 23, 45, 564, tzinfo=pytz.utc)
            time_2015_8 = datetime.datetime(
                2015, 8, 22, 11, 23, 45, 564, tzinfo=pytz.timezone("Asia/Shanghai"))
        except:
        ...
        self.assertEqual(Article.objects.archive(), [
                         (time_now.year, [time_now.month]), (2015, [8, 6]), (2011, [8])])
